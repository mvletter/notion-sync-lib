...
def test_5_diff_delete(master_page):
    ...
    blocks = [
        {
            "type": "paragraph",
            "paragraph": {
                "rich_text": [{"type": "text", "text": {"content": "DELETE ME - this will be removed"}}]
            }
        }
    ]
    append_blocks(client, master_page, blocks)
    ...
    from notion_sync import extract_block_text
    delete_block = [b for b in current_blocks if extract_block_text(b) == "DELETE ME - this will be removed"][0]
...
def test_6_diff_insert(master_page):
    ...
    # Add "INSERTED BLOCK" as new paragraph
    new_blocks = current_blocks.copy()

    # Insert a new block (we'll add it at the end for simplicity)
    inserted_block = {
        "type": "paragraph",
        "paragraph": {
            "rich_text": [{"type": "text", "text": {"content": "INSERTED - new block via diff"}}]
        }
    }
...
def test_7_diff_replace(master_page):
    ...
    blocks = [
        {
            "type": "paragraph",
            "paragraph": {
                "rich_text": [{"type": "text", "text": {"content": "REPLACE TYPE - will become heading"}}]
            }
        }
    ]
    append_blocks(client, master_page, blocks)
    ...
    # Find the paragraph
    from notion_sync import extract_block_text
    para_block = [b for b in current_blocks if b["type"] == "paragraph" and extract_block_text(b) == "REPLACE TYPE - will become heading"][0]
